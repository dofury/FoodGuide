...
public class MainDiaryPost extends AppCompatActivity {
            ...
            new ActivityResultCallback<ActivityResult>() {
                ...
                public void onActivityResult(ActivityResult result) {
                    if(result.getResultCode() == Activity.RESULT_OK && result.getData() != null) {
                        ...
                        Glide.with(MainDiaryPost.this).load(uri).into(diaryImageView);
    ...
    private void uploader(PostInfo postInfo,String key) {
        if (uri != null) {
            Toast.makeText(MainDiaryPost.this, "다이어리를 등록중입니다.", Toast.LENGTH_SHORT).show();
            Toast.makeText(MainDiaryPost.this, "잠시만 기다려주세요.", Toast.LENGTH_SHORT).show();
                            ...
                            public void onSuccess(Uri uri) {
                                ...
                                databaseReference.child("UserAccount").child(userAccount.getIdToken()).child("food").child(food.getId()).child(key).setValue(postInfo).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            loaderLayout.setVisibility(View.GONE);
                                            Toast.makeText(MainDiaryPost.this, "다이어리가 등록되었습니다.", Toast.LENGTH_SHORT).show();
                                            foodLogUpdate();
                                            onBackPressed();
                                        } else {
                                            loaderLayout.setVisibility(View.GONE);
                                            Toast.makeText(MainDiaryPost.this, "오류가 발생했습니다.", Toast.LENGTH_SHORT).show();
                                        }
                                    }
                                });
